...
export async function deleteHomeworkAdmin(id: string): Promise<void> {
  const { error } = await supabase.from("homework").delete().eq("id", id);
  if (error) throw error;
}

export async function deleteCandlePrayerAdmin(id: string): Promise<void> {
  const { error } = await supabase.from("candle_prayers").delete().eq("id", id);
  if (error) throw error;
}
...
export async function fetchAllHomework(): Promise<Homework[]> {
  const { data, error } = await supabase
    .from("homework")
    .select("*, profiles!homework_user_id_fkey(display_name)")
    .order("created_at", { ascending: false });
  if (error) throw error;
  return data || [];
}

export async function fetchAllCandlePrayers(): Promise<CandlePrayer[]> {
  const { data, error } = await supabase
    .from("candle_prayers")
    .select("*, profiles!candle_prayers_user_id_fkey(display_name)")
    .order("created_at", { ascending: false });
  if (error) throw error;
  return data || [];
}
